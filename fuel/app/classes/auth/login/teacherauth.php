...
class Auth_Login_Teacherauth extends \Auth\Auth_Login_Driver
{
	public static function _init()
	{
		\Config::load('teacherauth', true);

		// setup the remember-me session object if needed
		if (\Config::get('teacherauth.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('teacherauth.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('teacherauth.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	/**
	 * @var  array  TeacherAuth class config
	 */
	protected $config = array(
		'drivers' => array('group' => array('Teachergroup')),
	...
	protected function perform_check()
	{
		...
		$username    = \Session::get('t_username');
		$login_hash  = \Session::get('t_login_hash');
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
				$this->user = \DB::select_array(\Config::get('teacherauth.table_columns', array('*')))
				                 ->where('username', '=', $username)
				                 ->from(\Config::get('teacherauth.table_name'))
				                 ->execute(\Config::get('teacherauth.db_connection'))->current();
			}

			// return true when login was verified, and either the hash matches or multiple logins are allowed
			if ($this->user and (\Config::get('teacherauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
		...
		$this->user = \Config::get('teacherauth.guest_login', true) ? static::$guest_login : false;
		\Session::delete('t_username');
		\Session::delete('t_login_hash');
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('teacherauth.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('teacherauth.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('teacherauth.table_columns', array('*')))
		           ->where_open()
	...
	public function login($username_or_email = '', $password = '')
	{
		...
		\Session::set('t_username', $this->user['username']);
		\Session::set('t_login_hash', $this->create_login_hash());
	...
	public function force_login($user_id = '')
	{
		...
		$this->user = \DB::select_array(\Config::get('teacherauth.table_columns', array('*')))
		                 ...
		                 ->where_close()
		                 ->from(\Config::get('teacherauth.table_name'))
		                 ->execute(\Config::get('teacherauth.db_connection'))
		                 ->current();
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$same_users = \DB::select_array(\Config::get('teacherauth.table_columns', array('*')))
		                 ->where('username', '=', $username)
	...
	public function update_user($values, $username = null)
	{
		...
		$current_values = \DB::select_array(\Config::get('teacherauth.table_columns', array('*')))
		                     ->where('username', '=', $username)
		                     ->from(\Config::get('teacherauth.table_name'))
		                     ->execute(\Config::get('teacherauth.db_connection'));
		...
		$affected_rows = \DB::update(\Config::get('teacherauth.table_name'))
		                    ->set($update)
		                    ->where('username', '=', $username)
		                    ->execute(\Config::get('teacherauth.db_connection'));
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('teacherauth.table_name'))
		                    ->where('username', '=', $username)
		                    ->execute(\Config::get('teacherauth.db_connection'));
	...
	public function create_login_hash()
	{
		...
		$login_hash = sha1(\Config::get('teacherauth.login_hash_salt').$this->user['username'].$last_login);

		\DB::update(\Config::get('teacherauth.table_name'))
	...
	public function get_groups()
	{
		...
		return array(array('Teachergroup', $this->user['group']));
	}
	...
	{
		return \Config::get('teacherauth.guest_login', true);
	}

	public function create_teacher($username, $password, $birthday, $email, $gender, $first_name,
		$first_name_kana, $last_name, $last_name_kana, $group, Array $profile_fields = array())
	{
		$user = array(
			'username'        => (string) $username,
			'password'        => $this->hash_password((string) $password),
			...
			'first_name_kana' => (string) $first_name_kana,
			'last_name'       => (string) $last_name,
			'last_name_kana'  => (string) $last_name_kana,
			'profile_fields'  => serialize($profile_fields),
			'last_login'      => 0,
			'login_hash'      => '',
			'created_at'      => \Date::forge()->get_timestamp(),

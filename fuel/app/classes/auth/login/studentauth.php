...
class Auth_Login_Studentauth extends \Auth\Auth_Login_Driver
{
	public static function _init()
	{
		...
		if (\Config::get('studentauth.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('studentauth.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('studentauth.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	protected $config = array(
		'drivers' => array('group' => array('Studentgroup')),
	...
	protected function perform_check()
	{
		...
		$login_hash  = \Session::get('s_login_hash');
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
				$this->user = \DB::select_array(\Config::get('studentauth.table_columns', array('*')))
				                 ->where('username', '=', $username)
				                 ->from(\Config::get('studentauth.table_name'))
				                 ->execute(\Config::get('studentauth.db_connection'))->current();
			}

			// return true when login was verified, and either the hash matches or multiple logins are allowed
			if ($this->user and (\Config::get('studentauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
		...
		$this->user = \Config::get('studentauth.guest_login', true) ? static::$guest_login : false;
		\Session::delete('s_username');
		\Session::delete('s_login_hash');
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('studentauth.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('studentauth.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('studentauth.table_columns', array('*')))
		           ->where_open()
	...
	public function login($username_or_email = '', $password = '')
	{
		...
		\Session::set('s_username', $this->user['username']);
		\Session::set('s_login_hash', $this->create_login_hash());
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$result = \DB::insert(\Config::get('studentauth.table_name'))
		             ->set($user)
		             ->execute(\Config::get('studentauth.db_connection'));
	...
	public function update_user($values, $username = null)
	{
		...
		$current_values = \DB::select_array(\Config::get('studentauth.table_columns', array('*')))
		                     ->where('username', '=', $username)
		                     ->from(\Config::get('studentauth.table_name'))
		                     ->execute(\Config::get('studentauth.db_connection'));
		...
		$affected_rows = \DB::update(\Config::get('studentauth.table_name'))
		                    ->set($update)
		                    ->where('username', '=', $username)
		                    ->execute(\Config::get('studentauth.db_connection'));
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('studentauth.table_name'))
		                    ->where('username', '=', $username)
		                    ->execute(\Config::get('studentauth.db_connection'));
	...
	public function create_login_hash()
	{
		...
		$login_hash = sha1(\Config::get('studentauth.login_hash_salt').$this->user['username'].$last_login);

		\DB::update(\Config::get('studentauth.table_name'))
	...
	public function get_groups()
	{
		...
		return array(array('Studentgroup', $this->user['group']));
	}
	...
	{
		return \Config::get('studentauth.guest_login', true);
	}

	public function create_student($username, $password, $birthday, $email, $gender, $first_name,
		$first_name_kana, $last_name, $last_name_kana, $class_id, Array $profile_fields = array())
	{
		$user = array(
			'username'        => (string) $username,
			'password'        => $this->hash_password((string) $password),
